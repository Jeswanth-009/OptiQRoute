...
async fn upload_osm(
    ...
    while let Some(field) = multipart.next_field().await.map_err(|e| {
        ...
        HandlerError::ParseError(format!("Failed to read multipart data: {} (This might be due to file size limits or corrupted data)", e))
    })? {
        ...
        match name.as_str() {
            "file" => {
                ...
                let data = field.bytes().await.map_err(|e| {
                    info!("Failed to read file bytes: {}", e);
                    HandlerError::ParseError(format!("Failed to read file data: {} (File might be too large or corrupted)", e))
                })?;
                info!("Successfully read {} bytes from file field", data.len());
